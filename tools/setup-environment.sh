...
if [ -d "$CWD/sources/meta-boot2qt" ]; then
  ...
  TNCONFIGS=$(ls $CWD/sources/meta-curiosis/conf/machine/*.conf | xargs -n 1 basename | grep -E -c "$MACHINE")
fi
...
if [ -n "${DISTRO}" ]; then
  if [ ${TNCONFIGS} != 0 ]; then
    ...
    echo "    TEMPLATECONF=$CWD/sources/meta-curiosis/conf MACHINE=$MACHINE DISTRO=$DISTRO source $PROGNAME $BUILDDIRECTORY"
    echo ""
    TEMPLATECONF="$CWD/sources/meta-curiosis/conf" MACHINE=$MACHINE DISTRO=$DISTRO source $PROGNAME $BUILDDIRECTORY
...
if [ ${TNCONFIGS} != 0 -o ${FSLCONFIGS} != 0 ] ; then
  ...
  if [ -d $PWD/../sources/meta-curiosis ]; then
    if ! grep -Fq "meta-curiosis" $PWD/conf/bblayers.conf; then
      echo "" >> $PWD/conf/bblayers.conf
      echo "# setup TechNexion i.MX Yocto Project Release Layers in bblayers.conf" | tee -a $PWD/conf/bblayers.conf
      echo "BBLAYERS += \" \${BSPDIR}/sources/meta-curiosis \"" >> $PWD/conf/bblayers.conf
    fi
  fi
...
EOF
    ...
    if ! grep -Fq "meta-curiosis/recipes-containers/docker-disk/docker-disk.bb" $PWD/conf/local.conf; then
      echo "BBMASK += \"meta-curiosis/recipes-containers/docker-disk/docker-disk.bb\"" >> $PWD/conf/local.conf
    fi
    if ! grep -Fq "meta-curiosis/recipes-containers/docker/docker-ce_%.bbappend" $PWD/conf/local.conf; then
      echo "BBMASK += \"meta-curiosis/recipes-containers/docker/docker-ce_%.bbappend\"" >> $PWD/conf/local.conf
    fi
    # for boot2qt
    if grep -Fq "DISTRO ?= \"b2qt\"" $PWD/conf/local.conf; then
      if ! grep -Fq "meta-curiosis/recipes-graphics/wayland/weston_%.bbappend" $PWD/conf/local.conf; then
        echo "BBMASK += \"meta-curiosis/recipes-graphics/wayland/weston_%.bbappend\"" >> $PWD/conf/local.conf
      fi
      if ! grep -Fq "meta-curiosis/recipes-qt/qt5/qtbase_%.bbappend" $PWD/conf/local.conf; then
        echo "BBMASK += \"meta-curiosis/recipes-qt/qt5/qtbase_%.bbappend\"" >> $PWD/conf/local.conf
      fi
    fi
  fi
fi
